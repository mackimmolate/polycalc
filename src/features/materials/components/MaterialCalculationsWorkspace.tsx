...
function toDraft(calculation: MaterialCalculation): CalculationDraft {
  return {
    id: calculation.id,
    label: calculation.label,
    kgMaterialInput: formatInputDecimal(calculation.kgMaterial, 3),
    printTimeHoursInput: formatInputDecimal(calculation.printTimeHours, 2),
    saving: false,
    deleting: false,
    error: null,
  };
  ...
  useEffect(() => {
    ...
    const loadCalculations = async () => {
      ...
      try {
        ...
        if (isMounted) {
          setDrafts(calculations.map(toDraft));
        }
      } catch (caughtError) {
        if (isMounted) {
          setError(
  ...
  const saveCalculation = async (draft: CalculationDraft) => {
    ...
    try {
      ...
      updateDraft(draft.id, (current) => ({
        ...current,
        label: updated.label,
        kgMaterialInput: formatInputDecimal(updated.kgMaterial, 3),
        printTimeHoursInput: formatInputDecimal(updated.printTimeHours, 2),
        saving: false,
        error: null,
      }));
    } catch (caughtError) {
              ...
              <li key={draft.id}>
                <SurfaceCard className="space-y-3 border-[var(--border)] bg-white">
                  ...
                  <div className="space-y-1 rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] px-3 py-2">
                    ...
                    <p className="text-xs text-[var(--muted)]">Printtid: {printTimeLabel}</p>
                  </div>
